...
std::string buildUrlPreviewHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildUrlPreviewHtml"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildUrlPreviewHtml" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatUrlForDisplay(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatUrlForDisplay"})";
