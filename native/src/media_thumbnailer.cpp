...
std::string generateThumbnail(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"generateThumbnail"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "generateThumbnail" << R"(","ok":true)";
...
std::string getThumbnailSize(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getThumbnailSize" << R"(","ok":true)";
...
std::string isAnimated(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isAnimated" << R"(","result":)" << (result ? "true" : "false");
...
std::string getMimeForThumb(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMimeForThumb" << R"(","ok":true)";
...
std::string cacheThumbnailKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"cacheThumbnailKey"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cacheThumbnailKey" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
