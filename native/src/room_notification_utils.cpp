...
std::string buildRoomNotifOverride(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRoomNotifOverride"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRoomNotifOverride" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string buildRoomNotifRemove(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRoomNotifRemove"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRoomNotifRemove" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
    o << R"(,"content_size":)" << content.size();
    if (!format.empty()) o << R"(,"format":")" << format << R"(")";
    if (ts > 0) o << R"(,"timestamp":)" << ts;
...
std::string formatNotifLevel(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatNotifLevel"})";
