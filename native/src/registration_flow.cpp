...
std::string parseRegistrationStages(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseRegistrationStages"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseRegistrationStages" << R"(","parsed":true)";
...
std::string buildRegistrationBody(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRegistrationBody"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRegistrationBody" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string validateUsername(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateUsername"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateUsername" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string checkPasswordPolicy(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkPasswordPolicy"})";
    std::string input = parseJsonStringValue(json, "input");
    if (input.empty()) input = parseJsonStringValue(json, "value");
    if (input.empty()) input = json;
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkPasswordPolicy" << R"(","valid":)" << (valid ? "true" : "false");
    if (!error.empty()) o << R"(,"error":")" << error << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << R"(,"alnum_count":)" << std::count_if(input.begin(), input.end(),
        [](unsigned char c) { return std::isalnum(c); });
...
std::string formatRegError(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatRegError"})";
