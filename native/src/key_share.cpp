...
std::string requestKeyShare(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"requestKeyShare"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "requestKeyShare" << R"(","ok":true)";
...
std::string processKeyShare(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"processKeyShare"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "processKeyShare" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string isKeyKnown(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isKeyKnown" << R"(","result":)" << (result ? "true" : "false");
...
std::string getMissingSessions(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMissingSessions" << R"(","ok":true)";
...
std::string formatKeyShareEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatKeyShareEvent"})";
