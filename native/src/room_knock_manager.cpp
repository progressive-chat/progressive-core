...
std::string sendKnock(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"sendKnock"})";
...
std::string parseKnockResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseKnockResponse"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseKnockResponse" << R"(","parsed":true)";
...
std::string canKnock(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canKnock" << R"(","result":)" << (result ? "true" : "false");
...
std::string getPendingKnocks(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getPendingKnocks" << R"(","ok":true)";
...
std::string cancelKnock(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cancelKnock" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
