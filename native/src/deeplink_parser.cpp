...
std::string parseMatrixTo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseMatrixTo"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseMatrixTo" << R"(","parsed":true)";
...
std::string parseMatrixLink(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseMatrixLink"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseMatrixLink" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string isValidDeeplink(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidDeeplink" << R"(","result":)" << (result ? "true" : "false");
...
std::string extractRoomId(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractRoomId" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string formatDeeplinkRedirect(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatDeeplinkRedirect"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatDeeplinkRedirect" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
