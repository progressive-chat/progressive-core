...
std::string scheduleNextSync(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"scheduleNextSync"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "scheduleNextSync" << R"(","ok":true)";
...
std::string getRetryDelay(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRetryDelay" << R"(","ok":true)";
...
std::string shouldSyncNow(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "shouldSyncNow" << R"(","result":)" << (result ? "true" : "false");
...
std::string resetSyncTimer(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"resetSyncTimer"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "resetSyncTimer" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string formatSyncSchedule(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSyncSchedule"})";
