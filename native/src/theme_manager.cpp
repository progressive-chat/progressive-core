...
std::string parseTheme(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseTheme"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseTheme" << R"(","parsed":true)";
...
std::string applyDarkMode(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"applyDarkMode"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "applyDarkMode" << R"(","ok":true)";
...
std::string getAccentColor(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getAccentColor" << R"(","ok":true)";
...
std::string isSystemTheme(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isSystemTheme" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatThemeCss(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatThemeCss"})";
