...
std::string getMemberCount(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMemberCount" << R"(","ok":true)";
...
std::string incrementMemberCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"incrementMemberCount"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "incrementMemberCount" << R"(","ok":true)";
...
std::string decrementMemberCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"decrementMemberCount"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "decrementMemberCount" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string clearMemberCache(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"clearMemberCache"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "clearMemberCache" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string isCountStale(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isCountStale" << R"(","result":)" << (result ? "true" : "false");
