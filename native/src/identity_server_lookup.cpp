...
std::string lookupIdentity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"lookupIdentity"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "lookupIdentity" << R"(","ok":true)";
...
std::string bindIdentity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"bindIdentity"})";
...
std::string unbindIdentity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"unbindIdentity"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "unbindIdentity" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string formatIdentityCard(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatIdentityCard"})";
