...
std::string exportRoomKeys(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"exportRoomKeys"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "exportRoomKeys" << R"(","ok":true)";
...
std::string importRoomKeys(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"importRoomKeys"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "importRoomKeys" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string parseKeyFile(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseKeyFile"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseKeyFile" << R"(","parsed":true)";
...
std::string verifyKeyPassword(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifyKeyPassword"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifyKeyPassword" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string formatKeyExport(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatKeyExport"})";
