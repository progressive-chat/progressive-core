...
std::string validateInvite(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateInvite"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateInvite" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string isExpired(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isExpired" << R"(","result":)" << (result ? "true" : "false");
...
std::string canAccept(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canAccept" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string getInviteDetails(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getInviteDetails" << R"(","ok":true)";
...
std::string formatInviteSummary(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatInviteSummary"})";
