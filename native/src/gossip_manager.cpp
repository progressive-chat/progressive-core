...
std::string gossipKeyRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"gossipKeyRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "gossipKeyRequest" << R"(","ok":true)";
...
std::string receiveGossip(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"receiveGossip"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "receiveGossip" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getPendingGossips(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getPendingGossips" << R"(","ok":true)";
...
std::string cancelGossip(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cancelGossip" << R"(","result":)" << (result ? "true" : "false");
...
std::string gossipCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"gossipCount"})";
