...
std::string calculateOffset(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"calculateOffset"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "calculateOffset" << R"(","ok":true)";
...
std::string buildPaginationToken(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildPaginationToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildPaginationToken" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parsePaginationToken(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parsePaginationToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parsePaginationToken" << R"(","parsed":true)";
...
std::string mergePaginatedResults(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"mergePaginatedResults"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "mergePaginatedResults" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
