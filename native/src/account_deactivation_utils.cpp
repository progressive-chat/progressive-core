...
std::string parseDeactivation(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseDeactivation"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseDeactivation" << R"(","parsed":true)";
...
std::string buildDeactivationBody(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildDeactivationBody"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildDeactivationBody" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string validatePassword(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validatePassword"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validatePassword" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string checkPendingRequests(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkPendingRequests"})";
    std::string input = parseJsonStringValue(json, "input");
    if (input.empty()) input = parseJsonStringValue(json, "value");
    if (input.empty()) input = json;
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkPendingRequests" << R"(","valid":)" << (valid ? "true" : "false");
    if (!error.empty()) o << R"(,"error":")" << error << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << R"(,"alnum_count":)" << std::count_if(input.begin(), input.end(),
        [](unsigned char c) { return std::isalnum(c); });
...
std::string formatDeactivationConfirm(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatDeactivationConfirm"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatDeactivationConfirm" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
