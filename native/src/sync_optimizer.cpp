...
std::string mergeSyncResponses(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"mergeSyncResponses"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "mergeSyncResponses" << R"(","ok":true)";
...
std::string dedupeEvents(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"dedupeEvents"})";
...
std::string prioritizeRoom(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"prioritizeRoom"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "prioritizeRoom" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string shouldSkipRoom(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "shouldSkipRoom" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatSyncStats(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSyncStats"})";
