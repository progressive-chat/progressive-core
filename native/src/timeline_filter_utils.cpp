...
std::string parseFilter(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseFilter"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseFilter" << R"(","parsed":true)";
...
std::string applyFilterToTimeline(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"applyFilterToTimeline"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "applyFilterToTimeline" << R"(","ok":true)";
...
std::string getFilteredCount(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getFilteredCount" << R"(","ok":true)";
...
std::string isEventFiltered(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isEventFiltered" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatFilterLabel(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatFilterLabel"})";
