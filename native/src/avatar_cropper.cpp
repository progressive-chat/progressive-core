...
std::string parseCropRegion(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseCropRegion"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseCropRegion" << R"(","parsed":true)";
...
std::string computeCrop(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"computeCrop"})";
...
std::string applyCrop(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"applyCrop"})";
...
std::string validateAspectRatio(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateAspectRatio"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateAspectRatio" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string formatCropParams(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatCropParams"})";
