...
std::string parseConfig(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseConfig"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseConfig" << R"(","parsed":true)";
...
std::string getCustomConfig(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getCustomConfig" << R"(","ok":true)";
...
std::string isFeatureEnabled(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isFeatureEnabled" << R"(","result":)" << (result ? "true" : "false");
...
std::string reloadConfig(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"reloadConfig"})";
