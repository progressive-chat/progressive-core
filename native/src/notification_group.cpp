...
std::string groupByRoom(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"groupByRoom"})";
...
std::string isGrouped(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isGrouped" << R"(","result":)" << (result ? "true" : "false");
...
std::string getGroupSummary(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getGroupSummary" << R"(","ok":true)";
...
std::string shouldGroupMore(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "shouldGroupMore" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatGroupNotification(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatGroupNotification"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatGroupNotification" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
