...
std::string startBroadcast(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"startBroadcast"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "startBroadcast" << R"(","ok":true)";
...
std::string stopBroadcast(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"stopBroadcast"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "stopBroadcast" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getBroadcastState(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getBroadcastState" << R"(","ok":true)";
...
std::string getListenerCount(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getListenerCount" << R"(","ok":true)";
...
std::string formatBroadcastChunk(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBroadcastChunk"})";
