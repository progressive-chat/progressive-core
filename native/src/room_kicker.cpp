...
std::string canKickUser(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canKickUser" << R"(","result":)" << (result ? "true" : "false");
...
std::string buildKickEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildKickEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildKickEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parseKickResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseKickResponse"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseKickResponse" << R"(","parsed":true)";
...
std::string getKickReason(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getKickReason" << R"(","ok":true)";
...
std::string formatKickNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatKickNotice"})";
