...
std::string buildForwardRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildForwardRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildForwardRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string buildForwardContent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildForwardContent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildForwardContent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
    o << R"(,"content_size":)" << content.size();
    if (!format.empty()) o << R"(,"format":")" << format << R"(")";
    if (ts > 0) o << R"(,"timestamp":)" << ts;
