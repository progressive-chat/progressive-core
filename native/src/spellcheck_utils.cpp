...
std::string loadDictionary(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"loadDictionary"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "loadDictionary" << R"(","ok":true)";
...
std::string checkSpelling(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkSpelling"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkSpelling" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string getSuggestions(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSuggestions" << R"(","ok":true)";
...
std::string tokenizeForSpelling(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"tokenizeForSpelling"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "tokenizeForSpelling" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getMisspelledWords(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMisspelledWords" << R"(","ok":true)";
