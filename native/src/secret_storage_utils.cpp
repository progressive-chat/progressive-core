...
std::string encryptSecret(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"encryptSecret"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "encryptSecret" << R"(","ok":true)";
...
std::string decryptSecret(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"decryptSecret"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "decryptSecret" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string parseSecretStorageKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSecretStorageKey"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSecretStorageKey" << R"(","parsed":true)";
...
std::string buildSecretStorageEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildSecretStorageEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildSecretStorageEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string getDefaultKeyId(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getDefaultKeyId" << R"(","ok":true)";
