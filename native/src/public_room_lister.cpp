...
std::string parsePublicRooms(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parsePublicRooms"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parsePublicRooms" << R"(","parsed":true)";
...
std::string filterBySearch(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"filterBySearch"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "filterBySearch" << R"(","ok":true)";
...
std::string sortByPopularity(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"sortByPopularity"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "sortByPopularity" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string paginateRooms(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"paginateRooms"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "paginateRooms" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string formatRoomDirectoryEntry(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatRoomDirectoryEntry"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatRoomDirectoryEntry" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
