...
std::string startVerification(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"startVerification"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "startVerification" << R"(","ok":true)";
...
std::string acceptVerification(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"acceptVerification"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "acceptVerification" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string cancelVerification(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cancelVerification" << R"(","result":)" << (result ? "true" : "false");
...
std::string parseVerificationEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseVerificationEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseVerificationEvent" << R"(","parsed":true)";
...
std::string formatVerificationEmoji(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatVerificationEmoji"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatVerificationEmoji" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
