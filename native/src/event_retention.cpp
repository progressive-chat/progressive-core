...
std::string shouldRetain(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "shouldRetain" << R"(","result":)" << (result ? "true" : "false");
...
std::string getRetentionPeriod(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRetentionPeriod" << R"(","ok":true)";
...
std::string pruneOldEvents(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"pruneOldEvents"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "pruneOldEvents" << R"(","ok":true)";
...
std::string calculateAge(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"calculateAge"})";
...
std::string buildRetentionPolicy(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRetentionPolicy"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRetentionPolicy" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
