...
std::string signOutSession(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"signOutSession"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "signOutSession" << R"(","ok":true)";
...
std::string clearLocalData(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"clearLocalData"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "clearLocalData" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getSessionList(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSessionList" << R"(","ok":true)";
...
std::string isCurrentSession(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isCurrentSession" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatSignoutConfirmation(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSignoutConfirmation"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatSignoutConfirmation" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
