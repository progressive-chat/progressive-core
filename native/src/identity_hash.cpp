...
std::string hashIdentifier(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "hashIdentifier" << R"(","result":)" << (result ? "true" : "false");
...
std::string lookupHash(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"lookupHash"})";
...
std::string verifyHashedBinding(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifyHashedBinding"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifyHashedBinding" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string buildInviteWithHash(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildInviteWithHash"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildInviteWithHash" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string isHashValid(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isHashValid" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
