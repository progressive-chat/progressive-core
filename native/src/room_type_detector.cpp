...
std::string parseRoomType(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseRoomType"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseRoomType" << R"(","parsed":true)";
...
std::string isSpace(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isSpace" << R"(","result":)" << (result ? "true" : "false");
...
std::string isDirect(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isDirect" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string isGroupRoom(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isGroupRoom" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatRoomTypeLabel(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatRoomTypeLabel"})";
