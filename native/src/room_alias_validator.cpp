...
std::string parseAlias(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseAlias"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseAlias" << R"(","parsed":true)";
...
std::string isValidAlias(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidAlias" << R"(","result":)" << (result ? "true" : "false");
...
std::string isLocalAlias(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isLocalAlias" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string getServerFromAlias(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getServerFromAlias" << R"(","ok":true)";
...
std::string normalizeAlias(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"normalizeAlias"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "normalizeAlias" << R"(","ok":true)";
