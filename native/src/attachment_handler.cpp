...
std::string parseAttachment(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseAttachment"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseAttachment" << R"(","parsed":true)";
...
std::string getThumbnailUrl(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getThumbnailUrl" << R"(","ok":true)";
...
std::string isEncryptedAttachment(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isEncryptedAttachment" << R"(","result":)" << (result ? "true" : "false");
...
std::string decryptAttachment(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"decryptAttachment"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "decryptAttachment" << R"(","ok":true)";
...
std::string buildAttachmentEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildAttachmentEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildAttachmentEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
