...
std::string getSidebarOrder(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSidebarOrder" << R"(","ok":true)";
...
std::string pinRoom(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"pinRoom"})";
...
std::string unpinRoom(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"unpinRoom"})";
...
std::string isPinned(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isPinned" << R"(","result":)" << (result ? "true" : "false");
...
std::string reorderSidebar(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"reorderSidebar"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "reorderSidebar" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
