...
std::string scheduleMute(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"scheduleMute"})";
...
std::string cancelMute(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cancelMute" << R"(","result":)" << (result ? "true" : "false");
...
std::string isMuted(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isMuted" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatMuteStatus(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatMuteStatus"})";
