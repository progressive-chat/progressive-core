...
std::string shareSecret(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"shareSecret"})";
...
std::string requestSecret(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"requestSecret"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "requestSecret" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string cancelSecretRequest(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "cancelSecretRequest" << R"(","result":)" << (result ? "true" : "false");
...
std::string parseSecretEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSecretEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSecretEvent" << R"(","parsed":true)";
...
std::string formatSecretNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSecretNotice"})";
