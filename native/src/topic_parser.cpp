...
std::string parseTopic(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseTopic"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseTopic" << R"(","parsed":true)";
...
std::string extractTopicLinks(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"extractTopicLinks"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractTopicLinks" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string formatTopicHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatTopicHtml"})";
...
std::string truncateTopic(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"truncateTopic"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "truncateTopic" << R"(","ok":true)";
...
std::string getTopicFallback(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getTopicFallback" << R"(","ok":true)";
