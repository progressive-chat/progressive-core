...
std::string computeThumbnailSize(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"computeThumbnailSize"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "computeThumbnailSize" << R"(","ok":true)";
...
std::string generateThumbnailUrl(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"generateThumbnailUrl"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "generateThumbnailUrl" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string parseThumbnailInfo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseThumbnailInfo"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseThumbnailInfo" << R"(","parsed":true)";
...
std::string getBestThumbnail(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getBestThumbnail" << R"(","ok":true)";
