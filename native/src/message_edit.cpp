...
std::string canEdit(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canEdit" << R"(","result":)" << (result ? "true" : "false");
...
std::string parseEditInfo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseEditInfo"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseEditInfo" << R"(","parsed":true)";
...
std::string formatEditNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatEditNotice"})";
...
std::string getLatestEdit(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getLatestEdit" << R"(","ok":true)";
