...
std::string buildWhoamiRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildWhoamiRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildWhoamiRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatSessionDebug(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSessionDebug"})";
