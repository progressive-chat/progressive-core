...
std::string blockUser(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"blockUser"})";
...
std::string unblockUser(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"unblockUser"})";
...
std::string isBlocked(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isBlocked" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatBlockNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBlockNotice"})";
