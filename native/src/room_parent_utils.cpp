...
std::string parseParentRoomId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseParentRoomId"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseParentRoomId" << R"(","parsed":true)";
...
std::string getParentSpaces(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getParentSpaces" << R"(","ok":true)";
...
std::string buildParentEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildParentEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildParentEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
