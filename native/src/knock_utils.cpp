...
std::string parseKnockEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseKnockEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseKnockEvent" << R"(","parsed":true)";
...
std::string buildKnockRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildKnockRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildKnockRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string checkKnockPermission(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkKnockPermission"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkKnockPermission" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string formatKnockMessage(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatKnockMessage"})";
...
std::string getKnockRules(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getKnockRules" << R"(","ok":true)";
