...
std::string parseVerificationRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseVerificationRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseVerificationRequest" << R"(","parsed":true)";
...
std::string acceptRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"acceptRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "acceptRequest" << R"(","ok":true)";
...
std::string declineRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"declineRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "declineRequest" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getRequestStatus(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRequestStatus" << R"(","ok":true)";
...
std::string formatRequestBanner(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatRequestBanner"})";
