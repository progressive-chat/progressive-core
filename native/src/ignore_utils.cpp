...
std::string parseIgnoreList(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseIgnoreList"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseIgnoreList" << R"(","parsed":true)";
...
std::string buildIgnoreEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildIgnoreEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildIgnoreEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string checkUserIgnored(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkUserIgnored"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkUserIgnored" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string formatIgnoredNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatIgnoredNotice"})";
...
std::string mergeIgnoreLists(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"mergeIgnoreLists"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "mergeIgnoreLists" << R"(","ok":true)";
