...
std::string parseStickerPack(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseStickerPack"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseStickerPack" << R"(","parsed":true)";
...
std::string getStickerUrl(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getStickerUrl" << R"(","ok":true)";
...
std::string isAnimatedSticker(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isAnimatedSticker" << R"(","result":)" << (result ? "true" : "false");
...
std::string searchStickers(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"searchStickers"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "searchStickers" << R"(","ok":true)";
...
std::string formatStickerHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatStickerHtml"})";
