...
std::string getMasterKeyStatus(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMasterKeyStatus" << R"(","ok":true)";
...
std::string isCrossSigningReady(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isCrossSigningReady" << R"(","result":)" << (result ? "true" : "false");
...
std::string getSSSSStatus(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSSSSStatus" << R"(","ok":true)";
...
std::string formatCrossSigningBanner(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatCrossSigningBanner"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatCrossSigningBanner" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
...
std::string needsSetup(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"needsSetup"})";
