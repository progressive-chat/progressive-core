...
std::string bootstrapCrossSign(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"bootstrapCrossSign"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "bootstrapCrossSign" << R"(","ok":true)";
...
std::string getBootstrapStatus(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getBootstrapStatus" << R"(","ok":true)";
...
std::string isBootstrapped(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isBootstrapped" << R"(","result":)" << (result ? "true" : "false");
...
std::string resetCrossSign(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"resetCrossSign"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "resetCrossSign" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string formatBootstrapStep(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBootstrapStep"})";
