...
std::string hashMessageContent(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "hashMessageContent" << R"(","result":)" << (result ? "true" : "false");
...
std::string verifyMessageHash(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifyMessageHash"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifyMessageHash" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string getHashAlgorithm(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getHashAlgorithm" << R"(","ok":true)";
...
std::string signHashedMessage(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"signHashedMessage"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "signHashedMessage" << R"(","ok":true)";
...
std::string extractHashFromEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"extractHashFromEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractHashFromEvent" << R"(","parsed":true)";
