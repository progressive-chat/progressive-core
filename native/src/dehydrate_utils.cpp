...
std::string parseDehydratedDevice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseDehydratedDevice"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseDehydratedDevice" << R"(","parsed":true)";
...
std::string buildDehydrationEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildDehydrationEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildDehydrationEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string checkDehydrationStatus(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkDehydrationStatus"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkDehydrationStatus" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string rehydrateDevice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"rehydrateDevice"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "rehydrateDevice" << R"(","ok":true)";
...
std::string exportPickleKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"exportPickleKey"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "exportPickleKey" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
