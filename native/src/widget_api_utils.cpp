...
std::string parseApiCall(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseApiCall"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseApiCall" << R"(","parsed":true)";
...
std::string buildApiResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildApiResponse"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildApiResponse" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string validateApiMethod(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateApiMethod"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateApiMethod" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string getSupportedApis(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSupportedApis" << R"(","ok":true)";
...
std::string formatApiError(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatApiError"})";
