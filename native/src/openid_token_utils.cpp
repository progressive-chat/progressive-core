...
std::string parseOpenIdToken(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseOpenIdToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseOpenIdToken" << R"(","parsed":true)";
...
std::string buildOpenIdRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildOpenIdRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildOpenIdRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string validateToken(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateToken" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string formatTokenInfo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatTokenInfo"})";
