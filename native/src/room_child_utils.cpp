...
std::string parseChildRoomId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseChildRoomId"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseChildRoomId" << R"(","parsed":true)";
...
std::string getChildrenRooms(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getChildrenRooms" << R"(","ok":true)";
...
std::string buildChildEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildChildEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildChildEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
