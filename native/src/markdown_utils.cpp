...
std::string parseMarkdownToHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseMarkdownToHtml"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseMarkdownToHtml" << R"(","parsed":true)";
...
std::string parseHtmlToPlain(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseHtmlToPlain"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseHtmlToPlain" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string stripMarkdownSyntax(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"stripMarkdownSyntax"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "stripMarkdownSyntax" << R"(","ok":true)";
...
std::string extractLinksFromMarkdown(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"extractLinksFromMarkdown"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractLinksFromMarkdown" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
