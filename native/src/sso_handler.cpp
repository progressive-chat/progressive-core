...
std::string parseSsoRedirect(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSsoRedirect"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSsoRedirect" << R"(","parsed":true)";
...
std::string buildSsoUrl(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildSsoUrl"})";
...
std::string extractTokenFromUrl(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"extractTokenFromUrl"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractTokenFromUrl" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string getProviderName(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getProviderName" << R"(","ok":true)";
...
std::string formatSsoLoading(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSsoLoading"})";
