...
std::string lookupEmail(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"lookupEmail"})";
...
std::string lookupPhone(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"lookupPhone"})";
...
std::string parseLookupResult(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseLookupResult"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseLookupResult" << R"(","parsed":true)";
...
std::string formatLookupNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatLookupNotice"})";
