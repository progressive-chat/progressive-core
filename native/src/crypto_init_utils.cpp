...
std::string initializeCrypto(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"initializeCrypto"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "initializeCrypto" << R"(","ok":true)";
...
std::string getCryptoVersion(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getCryptoVersion" << R"(","ok":true)";
...
std::string isCryptoAvailable(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isCryptoAvailable" << R"(","result":)" << (result ? "true" : "false");
...
std::string resetCrypto(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"resetCrypto"})";
...
std::string formatCryptoStatus(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatCryptoStatus"})";
