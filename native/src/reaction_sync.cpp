...
std::string syncReactions(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"syncReactions"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "syncReactions" << R"(","ok":true)";
...
std::string aggregateReactions(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"aggregateReactions"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "aggregateReactions" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string getMyReaction(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMyReaction" << R"(","ok":true)";
...
std::string removeReaction(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"removeReaction"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "removeReaction" << R"(","ok":true)";
    if (!id.empty()) o << R"(,"id":")" << id << R"(")";
...
std::string formatReactionCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatReactionCount"})";
