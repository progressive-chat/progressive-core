...
std::string parseUpgrade(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseUpgrade"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseUpgrade" << R"(","parsed":true)";
...
std::string getReplacementRoom(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getReplacementRoom" << R"(","ok":true)";
...
std::string isTombstoned(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isTombstoned" << R"(","result":)" << (result ? "true" : "false");
...
std::string buildUpgradeLink(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildUpgradeLink"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildUpgradeLink" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatUpgradePrompt(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatUpgradePrompt"})";
