...
std::string parseAccessRules(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseAccessRules"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseAccessRules" << R"(","parsed":true)";
...
std::string canUserJoin(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canUserJoin" << R"(","result":)" << (result ? "true" : "false");
...
std::string isRestricted(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isRestricted" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string buildAccessEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildAccessEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildAccessEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string getRecommendedAccessLevel(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRecommendedAccessLevel" << R"(","ok":true)";
