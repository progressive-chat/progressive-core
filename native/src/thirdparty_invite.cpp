...
std::string parseThirdpartyInvite(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseThirdpartyInvite"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseThirdpartyInvite" << R"(","parsed":true)";
...
std::string acceptThirdparty(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"acceptThirdparty"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "acceptThirdparty" << R"(","ok":true)";
...
std::string rejectThirdparty(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"rejectThirdparty"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "rejectThirdparty" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string isValidThirdparty(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidThirdparty" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatThirdpartyNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatThirdpartyNotice"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatThirdpartyNotice" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
