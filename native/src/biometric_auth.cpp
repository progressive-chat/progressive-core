...
std::string isBiometricAvailable(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isBiometricAvailable" << R"(","result":)" << (result ? "true" : "false");
...
std::string authenticate(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"authenticate"})";
...
std::string getBiometricType(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getBiometricType" << R"(","ok":true)";
...
std::string canUseBiometrics(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canUseBiometrics" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatBiometricPrompt(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBiometricPrompt"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatBiometricPrompt" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
