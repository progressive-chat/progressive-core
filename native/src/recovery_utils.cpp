...
std::string parseRecoveryKey(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseRecoveryKey"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseRecoveryKey" << R"(","parsed":true)";
...
std::string generateRecoveryPrompt(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"generateRecoveryPrompt"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "generateRecoveryPrompt" << R"(","ok":true)";
...
std::string verifyRecoveryPhrase(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifyRecoveryPhrase"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifyRecoveryPhrase" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string buildRecoveryEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRecoveryEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRecoveryEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string estimateRecoveryStrength(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"estimateRecoveryStrength"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "estimateRecoveryStrength" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
