...
std::string parseAvatarUrl(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseAvatarUrl"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseAvatarUrl" << R"(","parsed":true)";
...
std::string buildAvatarUpdateEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildAvatarUpdateEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildAvatarUpdateEvent" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
