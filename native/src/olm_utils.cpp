...
std::string createAccount(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "createAccount" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string verifySignature(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifySignature"})";
    std::string input = parseJsonStringValue(json, "input");
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifySignature" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string generateKeys(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"generateKeys"})";
