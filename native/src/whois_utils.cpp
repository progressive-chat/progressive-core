...
std::string buildWhoisRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildWhoisRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildWhoisRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatWhoisResult(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatWhoisResult"})";
