...
std::string scanMedia(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"scanMedia"})";
...
std::string isSafeContent(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isSafeContent" << R"(","result":)" << (result ? "true" : "false");
...
std::string getMediaDimensions(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getMediaDimensions" << R"(","ok":true)";
...
std::string detectBlurHash(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"detectBlurHash"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "detectBlurHash" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string formatMediaPreview(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatMediaPreview"})";
