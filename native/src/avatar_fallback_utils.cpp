...
std::string computeInitials(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"computeInitials"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "computeInitials" << R"(","ok":true)";
...
std::string getAvatarColor(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getAvatarColor" << R"(","ok":true)";
