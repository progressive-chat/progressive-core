...
std::string parseRoomPreview(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseRoomPreview"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseRoomPreview" << R"(","parsed":true)";
...
std::string getRoomHeroes(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRoomHeroes" << R"(","ok":true)";
...
std::string formatPreviewCard(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatPreviewCard"})";
...
std::string canPreviewRoom(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canPreviewRoom" << R"(","result":)" << (result ? "true" : "false");
