...
std::string parseLocationEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseLocationEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseLocationEvent" << R"(","parsed":true)";
...
std::string isLiveLocation(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isLiveLocation" << R"(","result":)" << (result ? "true" : "false");
...
std::string getExpiryTimestamp(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getExpiryTimestamp" << R"(","ok":true)";
...
std::string buildLocationUpdate(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildLocationUpdate"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildLocationUpdate" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatLocationPin(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatLocationPin"})";
