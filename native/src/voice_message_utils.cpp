...
std::string parseVoiceInfo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseVoiceInfo"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseVoiceInfo" << R"(","parsed":true)";
...
std::string isPlayed(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isPlayed" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatWaveform(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatWaveform"})";
...
std::string transcodeAudio(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"transcodeAudio"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "transcodeAudio" << R"(","ok":true)";
