...
std::string buildToastMessage(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildToastMessage"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildToastMessage" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string buildDelayedToast(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildDelayedToast"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildDelayedToast" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
    o << R"(,"content_size":)" << content.size();
    if (!format.empty()) o << R"(,"format":")" << format << R"(")";
    if (ts > 0) o << R"(,"timestamp":)" << ts;
...
std::string formatToastTime(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatToastTime"})";
