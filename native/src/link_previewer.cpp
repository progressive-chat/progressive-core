...
std::string parseLinkMetadata(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseLinkMetadata"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseLinkMetadata" << R"(","parsed":true)";
...
std::string isPreviewable(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isPreviewable" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatPreviewHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatPreviewHtml"})";
...
std::string cachePreview(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"cachePreview"})";
