...
std::string parseSessionInfo(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSessionInfo"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSessionInfo" << R"(","parsed":true)";
...
std::string buildSessionMetadata(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildSessionMetadata"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildSessionMetadata" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string getLastActiveTimestamp(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getLastActiveTimestamp" << R"(","ok":true)";
...
std::string formatDeviceFingerprint(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatDeviceFingerprint"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatDeviceFingerprint" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
...
std::string hashSessionId(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "hashSessionId" << R"(","result":)" << (result ? "true" : "false");
