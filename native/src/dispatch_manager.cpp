...
std::string dispatchEvent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"dispatchEvent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "dispatchEvent" << R"(","ok":true)";
...
std::string routeToHandler(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"routeToHandler"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "routeToHandler" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string prioritySort(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"prioritySort"})";
...
std::string canHandle(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canHandle" << R"(","result":)" << (result ? "true" : "false");
...
std::string getRegisteredHandlers(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getRegisteredHandlers" << R"(","ok":true)";
