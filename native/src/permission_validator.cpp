...
std::string checkRoomPermission(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkRoomPermission"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkRoomPermission" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string getPowerLevel(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getPowerLevel" << R"(","ok":true)";
...
std::string canSendMessage(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canSendMessage" << R"(","result":)" << (result ? "true" : "false");
...
std::string canInviteUser(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "canInviteUser" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatPermissionDenied(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatPermissionDenied"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "formatPermissionDenied" << R"(")";
    o << R"(,"len":)" << text.length();
    o << R"(,"words":)" << wordCount;
    o << R"(,"lines":)" << lineCount;
