...
std::string parseLoginToken(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseLoginToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseLoginToken" << R"(","parsed":true)";
...
std::string validateTokenExpiry(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateTokenExpiry"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateTokenExpiry" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string buildTokenLogin(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildTokenLogin"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildTokenLogin" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string formatTokenError(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatTokenError"})";
