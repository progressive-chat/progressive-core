...
std::string formatReadReceipts(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatReadReceipts"})";
...
std::string isFullyRead(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isFullyRead" << R"(","result":)" << (result ? "true" : "false");
...
std::string getLastReadEvent(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getLastReadEvent" << R"(","ok":true)";
...
std::string formatReadSummary(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatReadSummary"})";
