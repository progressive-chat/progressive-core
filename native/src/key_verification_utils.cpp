...
std::string parseVerificationStart(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseVerificationStart"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseVerificationStart" << R"(","parsed":true)";
...
std::string buildVerificationAccept(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildVerificationAccept"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildVerificationAccept" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parseKeyVerificationMac(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseKeyVerificationMac"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseKeyVerificationMac" << R"(","parsed":true)";
    if (!eventId.empty()) o << R"(,"event_id":")" << eventId << R"(")";
    if (!roomId.empty()) o << R"(,"room_id":")" << roomId << R"(")";
    if (!userId.empty()) o << R"(,"user_id":")" << userId << R"(")";
    if (!type.empty()) o << R"(,"type":")" << type << R"(")";
...
std::string validateSasEmoji(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateSasEmoji"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateSasEmoji" << R"(","valid":)" << (valid ? "true" : "false");
