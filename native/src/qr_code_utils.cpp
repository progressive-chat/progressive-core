...
std::string parseQrContent(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseQrContent"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseQrContent" << R"(","parsed":true)";
...
std::string buildQrPayload(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildQrPayload"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildQrPayload" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string verifyQrSignature(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"verifyQrSignature"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "verifyQrSignature" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string isValidQrFormat(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isValidQrFormat" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatQrHtml(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatQrHtml"})";
