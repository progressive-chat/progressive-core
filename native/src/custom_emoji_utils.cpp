...
std::string buildCustomEmojiImage(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildCustomEmojiImage"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildCustomEmojiImage" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string extractEmojiShortcode(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"extractEmojiShortcode"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "extractEmojiShortcode" << R"(","parsed":true)";
...
std::string replaceEmojiShortcodes(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"replaceEmojiShortcodes"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "replaceEmojiShortcodes" << R"(","ok":true)";
