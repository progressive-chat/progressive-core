...
std::string buildDeviceRenameRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildDeviceRenameRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildDeviceRenameRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parseDeviceRenameResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseDeviceRenameResponse"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseDeviceRenameResponse" << R"(","parsed":true)";
