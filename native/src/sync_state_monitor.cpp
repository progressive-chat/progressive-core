...
std::string isSyncing(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isSyncing" << R"(","result":)" << (result ? "true" : "false");
...
std::string getSyncProgress(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getSyncProgress" << R"(","ok":true)";
...
std::string getLastSyncTime(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getLastSyncTime" << R"(","ok":true)";
...
std::string formatSyncStatus(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatSyncStatus"})";
