...
std::string validateUserId(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateUserId"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateUserId" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string parseUserId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseUserId"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseUserId" << R"(","parsed":true)";
...
std::string buildUserId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildUserId"})";
...
std::string formatMxid(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatMxid"})";
...
std::string normalizeUserId(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"normalizeUserId"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "normalizeUserId" << R"(","ok":true)";
