...
std::string parseForgotten(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseForgotten"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseForgotten" << R"(","parsed":true)";
...
std::string markAsForgotten(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"markAsForgotten"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "markAsForgotten" << R"(","ok":true)";
...
std::string unforgetRoom(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"unforgetRoom"})";
...
std::string isForgotten(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isForgotten" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatForgotStatus(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatForgotStatus"})";
