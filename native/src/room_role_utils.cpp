...
std::string parseSuggestedRole(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSuggestedRole"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSuggestedRole" << R"(","parsed":true)";
...
std::string getDefaultRole(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getDefaultRole" << R"(","ok":true)";
...
std::string buildRoleSuggestion(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRoleSuggestion"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRoleSuggestion" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
