...
std::string buildMessageLink(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildMessageLink"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildMessageLink" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string buildUserLink(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildUserLink"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildUserLink" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
    o << R"(,"content_size":)" << content.size();
    if (!format.empty()) o << R"(,"format":")" << format << R"(")";
    if (ts > 0) o << R"(,"timestamp":)" << ts;
...
std::string buildRoomLink(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildRoomLink"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildRoomLink" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
    o << R"(,"content_size":)" << content.size();
    if (!format.empty()) o << R"(,"format":")" << format << R"(")";
    if (ts > 0) o << R"(,"timestamp":)" << ts;
