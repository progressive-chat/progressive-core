...
std::string calculateBadgeCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"calculateBadgeCount"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "calculateBadgeCount" << R"(","ok":true)";
...
std::string isHighlighted(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isHighlighted" << R"(","result":)" << (result ? "true" : "false");
...
std::string isNoisy(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isNoisy" << R"(","result":)" << (result ? "true" : "false");
    if (!matched.empty()) o << R"(,"matched":")" << matched << R"(")";
    if (!category.empty()) o << R"(,"category":")" << category << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << "}";
...
std::string formatBadge(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBadge"})";
...
std::string resetBadgeCount(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"resetBadgeCount"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "resetBadgeCount" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
