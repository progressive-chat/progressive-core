...
std::string parseServerNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseServerNotice"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseServerNotice" << R"(","parsed":true)";
...
std::string buildAdminCommand(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildAdminCommand"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildAdminCommand" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string validateAdminToken(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"validateAdminToken"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "validateAdminToken" << R"(","valid":)" << (valid ? "true" : "false");
...
std::string checkAdminPermission(const std::string& json) {
    if (json.empty()) return R"({"valid":false,"error":"empty input","fn":"checkAdminPermission"})";
    std::string input = parseJsonStringValue(json, "input");
    if (input.empty()) input = parseJsonStringValue(json, "value");
    if (input.empty()) input = json;
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "checkAdminPermission" << R"(","valid":)" << (valid ? "true" : "false");
    if (!error.empty()) o << R"(,"error":")" << error << R"(")";
    o << R"(,"input_length":)" << input.length();
    o << R"(,"alnum_count":)" << std::count_if(input.begin(), input.end(),
        [](unsigned char c) { return std::isalnum(c); });
...
std::string formatAdminResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatAdminResponse"})";
