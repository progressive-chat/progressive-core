...
std::string parseDisplayName(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseDisplayName"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseDisplayName" << R"(","parsed":true)";
...
std::string formatDisplayName(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatDisplayName"})";
...
std::string buildDisplayNameUpdate(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildDisplayNameUpdate"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildDisplayNameUpdate" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
