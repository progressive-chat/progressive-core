...
std::string detectBridge(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"detectBridge"})";
...
std::string getBridgeType(const std::string& json) {
    std::ostringstream o;
    o << R"({"fn":")" << "getBridgeType" << R"(","ok":true)";
...
std::string isBridgedRoom(const std::string& json) {
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "isBridgedRoom" << R"(","result":)" << (result ? "true" : "false");
...
std::string formatBridgeNotice(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"formatBridgeNotice"})";
...
std::string parseBridgeState(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseBridgeState"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseBridgeState" << R"(","parsed":true)";
