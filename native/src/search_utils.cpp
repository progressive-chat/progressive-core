...
std::string tokenizeQuery(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"tokenizeQuery"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "tokenizeQuery" << R"(","ok":true)";
...
std::string buildSearchBody(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildSearchBody"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildSearchBody" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parseSearchResults(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseSearchResults"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseSearchResults" << R"(","parsed":true)";
...
std::string rankSearchResults(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"rankSearchResults"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "rankSearchResults" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
...
std::string applySearchFilter(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"applySearchFilter"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "applySearchFilter" << R"(","ok":true)";
    o << R"(,"input_size":)" << input.size();
    if (!action.empty()) o << R"(,"action":")" << action << R"(")";
    if (!target.empty()) o << R"(,"target":")" << target << R"(")";
    if (flags != 0) o << R"(,"flags":)" << flags;
