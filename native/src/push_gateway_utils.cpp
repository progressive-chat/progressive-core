...
std::string buildPusherRequest(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildPusherRequest"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildPusherRequest" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
...
std::string parsePusherResponse(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parsePusherResponse"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parsePusherResponse" << R"(","parsed":true)";
