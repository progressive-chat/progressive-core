...
std::string parseActions(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"fn":"parseActions"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "parseActions" << R"(","parsed":true)";
...
std::string buildNotificationActions(const std::string& json) {
    if (json.empty()) return R"({"ok":false,"error":"empty input","fn":"buildNotificationActions"})";
    ...
    std::ostringstream o;
    o << R"({"fn":")" << "buildNotificationActions" << R"(","ok":true)";
    o << R"(,"type":")" << type << R"(")";
